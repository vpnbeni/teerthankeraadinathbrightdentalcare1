...
async function checkAppointments() {
  try {
    ...
    if (appointments.length === 0) {
      ...
      console.log(
        "1. Create some test appointments through the client interface"
      );
    ...
    const currentMonthAppointmentsRaw = await Appointment.find({
      date: {
        $gte: startOfMonth,
        $lte: endOfMonth,
      },
    ...
    // Filter out appointments with deleted users
    const currentMonthAppointments = currentMonthAppointmentsRaw.filter(
      (appointment) => appointment.userId !== null
    );

    console.log(
      `\n📊 Current month (${now.toLocaleDateString("en-US", {
        month: "long",
        year: "numeric",
    ...
    if (currentMonthAppointments.length > 0) {
      currentMonthAppointments.forEach((apt, index) => {
        console.log(
          `   ${index + 1}. ${apt.date.toISOString().split("T")[0]} ${
            apt.timeSlot
          } - ${apt.userId?.name || "Unknown"} (${apt.status})`
        );
      });
    }
  } catch (error) {
